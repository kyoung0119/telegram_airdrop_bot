...
SUBSCRIBE = mk(
    [
        ...
        [kb("$SCOT Channel 🚀", url="https://t.me/airdrop_updates_channel_test")],
        # [kb("$SCOT Group 👥", url="https://t.me/Scottishtalking")],
        [kb("$SCOT Group 👥", url="https://t.me/+jld7b2iupW5hOGNi")],
...
async def adding_pagination(
    buttons: list, page: int, elements_number: int, button_data: str
):
    ...
    if page > 0:
        if page > 1:
            ...
            pagination_button.append(
                kb(f"«« {page_minus_10 + 1}", button_data.format(page_minus_10))
            )
    ...
    if elements_number > Config.ELEMENTS_PER_PAGE:
        ...
        if elements_number > Config.ELEMENTS_PER_PAGE * 2:
            ...
            last_page = (
                elements_left // Config.ELEMENTS_PER_PAGE
                if elements_left % Config.ELEMENTS_PER_PAGE == 0
                else (elements_left // Config.ELEMENTS_PER_PAGE) + 1
            )
            page_plus_10 = page + 10 if last_page >= 10 else page + last_page

            pagination_button.append(
                kb(f"{page_plus_10 + 1} »»", button_data.format(page_plus_10))
            )
...
async def MembersMarkup(m: Message, page: int = 0):
    ...
    for user in users:
        bts = [
            kb(f"{user.name} ({user.balance})", "pass"),
            kb(
                "{}".format("🟢" if not user.is_banned else "🔴"),
                f"ban_status:{user.id}:{page}",
            ),
        ]
        markup.append(bts)
